# Book 2 — *The Shadow Over the Old Castle* (working art title)
## Storyboard Blueprint — 8 panels (+ optional 9th coda)
### v2 — RECONCILED TO THE POEM (`Three_Siblings_and_the_Shadow_Over_Hogwarts.md`)

**Status:** lettering now matches the verse. Lines marked **[verse]** are lifted (lightly
trimmed for bubble length); lines marked **[added]** are mine to motivate a beat — cut freely.
**Pipeline:** blueprint → per-panel ChatGPT prompts (constant block + scene + baked lettering) →
QA → optimize to webp → Claude Code commit. Same loop as Rome.

---

## ⚠️ IP: trademarked terms in the poem that must NEVER be painted into art
The poem uses these in its text; in the **images/lettering** use the generic substitute instead.
The internal *story* can keep them in our notes — only the painted panels must stay clean.

| In the poem | In the art (baked lettering + design) |
| --- | --- |
| "HOGWARTS" (the sign) | **the old castle** / Grimmoor / Hollowmere — **DECISION OPEN** |
| Harry, Ron, Hermione (named; glasses+scar) | three **generic original** young wizards, NO names, NO glasses-scar-bolt |
| "Devil's Snare" | "the vines" / "the snare-plant" (no proper name) |
| "Wingardium" | no spell word shown — just the troll tripping |
| "Dementors" | "the cold ones" / hooded shadow-wraiths |
| "EXPECTO PATRONUM" / "Patronus" | no incantation shown; glowing silver spirit-animals only |
| "Basilisk" | a giant serpent (basilisk is public-domain myth, but keep it generic) |
| "FINITE!" | no spell word — show the curse breaking visually |

---

## Director decisions baked into this draft (override freely)
1. **Panel count: 8 + optional 9.** The poem's ending (basilisk → curse broken → feast → home →
   spark) is huge. Draft makes the **basilisk/curse-break the climax (Panel 8)** and offers a
   **coda (Panel 9)** for feast + farewell + home + kept-spark. If you want the ending to breathe,
   9 could even split into 9 (feast/farewell) + 10 (home/spark).
2. **Outfits: Madrid trio in SIGNATURE MODERN OUTFITS, no robes** — preserves color-coded
   recognizability, contrasts them against the wizard allies, avoids the trademarked-uniform look.
3. **In-art school name: DECISION OPEN** — needed for Panel 2's sign. Default "the old castle."

## 🔄 CANON CHANGE (franchise-level) — Maria, logged this rev
**Maria is now curious, adventurous, and proactive** — the eldest who leans in and drives the
group forward. Retired: the "pretends to be unimpressed but secretly loves it" read, the
cool/aloof persona, and the **phone/camera motif** (no phone). This touches the LOCKED bible —
propagate to `CHATGPT_CONTEXT.md`, `HANDOFF.md`, and `characters/manifest.json`.
**Verse ripples to resolve (your call):** P1 line "Just a bore" -> a curious line; P7 "queen of
being cool" framing; P9 "no phone, no pose" payoff is now orphaned. Flagged inline below.

## Through-line: THE SILVER SPARK (confirmed by the verse)
The portal book glows **silver-blue**; the magic that saves the day is silver (Luna's fox);
Luna keeps a **tiny silver fox that stays** at the very end. Carry the silver-blue glow from
Panel 1 → peak at Panel 6 → payoff in the coda. It's the spine of the book.

---

## Reusable constant block (paste into EVERY Panel prompt)

> **ART STYLE (every panel):** soft hand-painted anime / gentle watercolor, warm lighting,
> delicate linework. Speech bubbles + narration box painted INTO the image, matching the art.
>
> **REFERENCE MODEL SHEETS (attach all 3 every time):** `maria_ref.png`, `pepe_ref.png`,
> `luna_ref.png` — treat as character model sheets. Goal: **recognizable & consistent,
> stylized — NOT photographic.** (Refs show blue shirts on white — ignore that wardrobe; use
> the outfit the prompt specifies.)
>
> **RENDER-AGE DIAL:** draw the cast as **"about 13, 12, and 12."** (Luna's canon age is 9;
> the number is only a size dial so she isn't drawn tiny.)
>
> **HEIGHT BAND (the #1 failure mode):** Maria 100% (tallest) · Pepe 97% (head at Maria's
> forehead) · Luna 93% (head at Maria's eyebrow). Three similarly-tall siblings in a NARROW
> band — NOT one big + two small. If unsure, taller not shorter.
>
> **SIGNATURE OUTFITS + FACE ANCHORS (state explicitly, never "may vary"):**
> - **Maria** — teal t-shirt, dark-blue jeans, teal sneakers. Long dark hair, center part with
>   a soft curtain fringe framing the face; **curious, adventurous, proactive** — the eldest who
>   leans in and leads the way (NOT aloof, never "unimpressed"). No phone motif.
> - **Pepe** — orange t-shirt with a cream horizontal stripe, green cargo shorts, blue
>   sneakers. Short tousled/spiky dark hair; animated, mischief-prone expressions.
> - **Luna** — yellow t-shirt, dark pants, brown shoes. Dark hair in a low ponytail with loose
>   front wisps; the largest, most expressive eyes (youngest read), big smiles.
>
> **IP GUARDRAILS:** original kids only. Generic original young wizards (NO glasses-scar-bolt
> iconography, no named characters). NO trademarked names, spell words, or designs baked into art.

---

## THE 8 PANELS

### Panel 1 — The Attic Portal  ✅ prompt written (p1.txt)
- **Verse beats:** thunderstorm; grim dusty book open on the attic floor; Maria pokes it; Pepe
  reads the words aloud; Luna touches the glowing page.
- **Setting:** dusty attic at night, lightning at the window, single dim bulb, cobwebs. The
  ancient grim book lies open on the floor giving a rising **silver-blue glow** (motif seed).
- **Staging:** Pepe crouched reading (instigator); Luna kneeling, hand reaching for the page,
  lit from below; Maria leaning in over the book, both hands free, bright and eager — drawn in,
  leading. NO phone.
- **Story bar:** running joke (Pepe causes it) + Luna's discovery.
- **Lettering:** Narr **[verse]** *"The thunder cracked. The lights went dim."* · Maria
  **[changed — profile]** *"What IS this?"* · Luna **[added]** *"Wait — it's GLOWING!"*

### Panel 2 — Arrival at the Old Castle
- **Verse beats:** they land in a clump on cold stone steps; cobwebbed halls; "Where ARE we?";
  the **sign**; blue torches; Maria's no-signal meltdown; a voice: "Phones won't help you here."
- **Setting:** vast cold gothic entrance hall, blue-flickering torches, cobwebbed corners. A
  weathered stone sign on the wall. **(Sign text = chosen original name + "ENTER IF YOU DARE.")**
- **Staging:** the three tumbled in a heap on the steps (Pepe under the pile — gag). Maria up
  first, shaking her phone in disbelief.
- **Story bar:** humor (the phone gag — Maria's character) + arrival dread.
- **Lettering:** Pepe **[verse]** *"Where ARE we?!"* · Sign **[verse, name-swapped]**
  *"___. ENTER IF YOU DARE."* · Maria **[verse]** *"No signal? NONE?!"* · off-panel voice
  bubble **[verse]** *"Phones won't help you here."*
- **IP:** do NOT paint "HOGWARTS" — use the decided name.

### Panel 3 — The Young Wizards
- **Verse beats:** three young wizards by candlelight, wands held tight; they introduce
  themselves; "The castle's cursed"; every monster loose; fight to save the school before light.
- **Setting:** candlelit stone corridor.
- **Staging:** the trio (height band) facing three **generic original** young wizards, one wand
  raised casting warm light. Wizard heights may vary.
- **Story bar:** alliance + the stakes (education: what's wrong with the castle).
- **Lettering:** Wizard **[verse, de-named]** *"The castle's cursed — every monster's loose."* ·
  Wizard **[verse]** *"We fight to save it before first light!"* · Maria **[added]** *"Cursed
  how, exactly?"*
- **IP:** generic wizards — NO names ("Harry/Ron/Hermione"), NO glasses+scar+bolt.

### Panel 4 — The Clutching Vines  *(Devil's Snare beat)*
- **Verse beats:** they slide into a pit; a toothed vine grabs Pepe's knee; "GET OFF OF ME!";
  "DON'T struggle — it feeds on fear! Go limp!"; they relax; *FWOOMP* the plant drops them.
- **Setting:** dark stone pit, writhing toothed vines.
- **Staging:** vine coiling Pepe's leg, Pepe flailing; a wizard calling the instruction; Maria &
  Luna going deliberately limp.
- **Story bar:** Pepe chaos + education beat (panic makes it worse).
- **Lettering:** Pepe **[verse]** *"GET OFF OF ME!"* · Wizard **[verse]** *"Don't struggle —
  go limp! It feeds on fear!"* · SFX **[verse]** *"FWOOMP!"*
- **IP:** call it "the vines," not "Devil's Snare."

### Panel 5 — The Troll
- **Verse beats:** BOOM BOOM; a ten-foot TROLL with club and stink; "Distract it!"; Pepe (brave
  & starving) lobs his last snack as bait; troll trips on a spell and bops its OWN head; *THUD*;
  "Just napping."
- **Setting:** crumbling stone chamber, rubble.
- **Staging:** big dumb troll mid-stumble, club swinging into its own head; Pepe mid-throw with a
  snack; kids ducking.
- **Story bar:** Pepe's hunger/clumsiness becomes the save (accidental heroics).
- **Lettering:** Wizard **[verse]** *"Distract it!"* · Pepe **[verse]** *"Have a snack!"* ·
  SFX **[verse]** *"THUD!"* · Wizard **[verse]** *"Just napping. Now RUN!"*
- **IP:** no spell word ("Wingardium") shown — just the trip.

### Panel 6 — The Cold Ones & Luna's Silver Fox  *(Dementor beat — Luna's hero moment)*
- **Verse beats:** air goes cold, torches die, hooded shapes glide (no faces); Luna feels the
  sadness; "Think of something JOYFUL!"; a wizard's glowing silver stag drives some back; but
  **Luna**, thinking of **gelato**, conjures a **tiny silver fox from her bare hands**; "She's
  got the spark!"; Luna: "I think I'm rather prepared."
- **Setting:** freezing corridor, frost on stone, hooded shadow-wraiths, light draining.
- **Staging:** Luna steps to the FRONT — most expressive — a glowing **silver fox** bursting
  from her open hands (NO wand), routing the wraiths. A wizard's silver spirit-animal can glow
  faintly behind. Maria & Pepe lit silver.
- **Story bar:** the emotional core — Luna's joy IS the weapon. Silver-spark motif peaks here.
- **Lettering:** Narr **[adapted]** *"The cold ones fed on happiness. Luna had plenty."* · Luna
  **[verse, trimmed]** *"I think… I'm rather prepared."* · Wizard **[verse]** *"She's got the
  spark!"*
- **IP:** hooded shadows = generic; NO "Dementor"/"Patronus"/incantation in art. Fox is original.

### Panel 7 — The Giant Spider & Maria's Fire  *(Maria's hero moment)*
- **Verse beats:** webbed corridors; a SPIDER big as a bed; Pepe: "I'm done. I'm out."; "They
  fear fire!"; Maria seizes a torch — "BACK OFF, you ghoul!"; spiders flee; "Did Maria just DO
  that?"; "Don't make it weird." (she glows with pride).
- **Setting:** web-choked dark corridor.
- **Staging:** huge spider recoiling; **Maria front and center** brandishing a flaming torch,
  fierce; Pepe gawking; Luna behind.
- **Story bar:** character payoff — proactive Maria seizing the torch fits the new profile even
  better. *(Ripple: drop the verse's "queen of being cool" framing — she's bold here, not aloof.
  "Don't make it weird" still works as sibling banter.)*
- **Lettering:** Pepe **[verse]** *"I'm done. I'm out."* · Maria **[verse]** *"BACK OFF, you
  ghoul!"* · Pepe **[verse]** *"…Did Maria just DO that?"* · Maria **[verse]** *"Don't make it
  weird."*

### Panel 8 — The Serpent & the Breaking of the Curse  *(CLIMAX)*
- **Verse beats:** the chamber's core; a train-long serpent whose stare can kill; "DON'T LOOK —
  eyes shut, win by sound!"; the six fight blind, hand-in-hand; Pepe baits it ("OVER HERE, you
  snake!" — chicken leg); Luna's fox-light blinds it; Maria hurls the troll's club; the curse
  breaks; the serpent turns to stone; the castle wakes, torches blaze, gloom rolls back.
- **Setting:** vast echoing marble chamber, the giant serpent coiled around the curse's dark
  heart-light.
- **Staging:** all six mid-climax, several with eyes squeezed shut; Pepe yelling with a chicken
  leg; Luna's silver fox-light flaring; Maria mid-throw; light flooding back as the curse shatters.
- **Story bar:** teamwork payoff — every trait that caused trouble now wins the day.
- **Lettering:** Wizard **[verse]** *"DON'T LOOK! Eyes shut!"* · Pepe **[verse]** *"OVER HERE,
  you snake!"* · Narr **[adapted]** *"The castle woke — alive inside."*
- **IP:** giant serpent kept generic; NO spell word ("Finite") shown — break the curse visually.

---

## Panel 9 — The Feast, Farewell & Home  *(OPTIONAL CODA — cut to match Rome's 8)*
- **Verse beats:** ghosts cheer; the Great Hall opens to a huge feast (Pepe's heaven); "Eat —
  you've earned it!"; Pepe devours plates; Maria smiles with no phone, no pose; Luna makes foxes
  dance; "You three could STAY"; the book reglows; hugs, "Come back!" "Eventually!"; *WHOOSH*
  home to the attic; the book snaps shut; Maria: "…that was kind of brill."; Pepe burps; Luna's
  **tiny silver fox that STAYS**: "I think we kept a little spark."
- **Setting:** split — golden feast in the woken hall → the dim attic, book snapped shut.
- **Staging:** feast = Pepe face-first in food (running joke). Final beat: attic, Luna's cupped
  hands holding one tiny silver fox/spark — same silver-blue from Panel 1, now warm.
- **Story bar:** running joke payoff + the quiet emotional button (the kept spark).
- **Lettering:** Pepe **[adapted]** *"Best. Feast. EVER."* · Maria **[verse]** *"…That was kind
  of brill."* · Luna **[verse, trimmed]** *"I think we kept a little spark."*
- **Note:** mirrors the silver-spark motif full circle (Panel 1 → 6 → 9).
- **Ripple (profile):** the verse's "no phone, no pose" payoff is now orphaned (no phone set up
  earlier). Suggest revising that coda line, e.g. *"Maria smiled, wide and unguarded — just like
  a child."*

---

## Reader page — "Show the poem" toggle (REUSE Rome's pattern; Claude Code build, later)
The Hogwarts reader (`stories/hogwarts/index.html`) reuses Rome's storybook toggle: a "Show the
poem" button in the top bar that flips the page into storybook mode, revealing a poem-stanza card
above each panel (default view stays the clean comic). Pure HTML/CSS/JS, no image changes.
**Architecture (as built in Rome — match it):** stanzas are NOT hardcoded in JS. Each panel entry
in `stories/hogwarts/pages.json` carries a `stanza` field holding that panel's verse, and
`index.html` reads the stanzas FROM pages.json (single source). Keep `stories/hogwarts/poem.md`
as a reference copy of the full poem. To revise the poem later, edit the `stanza` fields — one place.
**Principle (locked):** poem stanzas (narrator's verse) and baked bubbles (characters' live
dialogue) do NOT match word-for-word — that's the picture-book convention, intentional.

## Poem stanza → panel map (populate each panel's `stanza` field in pages.json)
The poem has more verses than panels, so each panel gets a stanza *block*. Anchors by first line:
- **P1** — "The thunder cracked…" → through "…freed from its cage," (book wakes, page touched).
- **P2** — "they tumbled, spun…" → through "…'Phones won't help you here.'" (land, sign, no signal).
- **P3** — "then *smiled.* For there…" → through "…before first light!'" (the wizards; the curse).
- **P4** — "'This way!' cried Ron…" → through "…the plant just dropped them back." (the vines).
- **P5** — "A *BOOM.* A *BOOM.*…" → through "…there's MUCH worse waiting there.'" (the troll).
- **P6** — "The air went cold…" → through "…I'm rather prepared.'" (cold ones; Luna's fox).
- **P7** — "Down corridors of web…" → through "…(She *glowed* with pride.)" (spider; Maria's fire).
- **P8** — "At last they reached…" → through "…Hogwarts WOKE: alive inside!" (basilisk; curse broken).
- **P9** (if used) — "The ghosts all cheered…" → through "…the bravest team that Hogwarts won."
- *If 8 panels:* fold the P9 block onto P8, or split P8's climax verse from the resolution verse.
- *Name in displayed stanza:* the verse says "HOGWARTS"; decide whether the reader stanza keeps it
  (it's the narrator's poem text) or swaps to the art's name. The ART still never shows it. Your call.

## Open items before we go past Panel 1
- [x] Reconcile narration/bubbles to the verse — DONE (this v2).
- [x] Reader poem-toggle pattern + stanza→panel map — captured (build via Claude Code later).
- [ ] **Lock in-art school name** (needed for Panel 2's sign): "the old castle" / Grimmoor /
      Hollowmere / other.
- [ ] Lock panel count: 8 vs 9 (vs 9+10 for a roomier ending).
- [ ] Maria: pick a replacement motif (explorer's notebook / none) + resolve the P7 & P9 verse ripples.
- [ ] Confirm Luna's added line in Panel 1 ("Wait — it's GLOWING!") stays or goes.
- [ ] Decide whether the 3 wizard allies recur enough to need their own mini-refs (panels 3–9).
